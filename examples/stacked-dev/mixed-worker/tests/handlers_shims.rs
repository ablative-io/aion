...
fn dev_parses_the_canned_dev_report() -> TestResult {
    let shims = Shims::new()?;
    shims.write("claude", &format!("printf '%s' '{DEV_REPORT_BODY}'"))?;
...
fn dev_unwraps_real_norns_output_envelope_ignoring_telemetry_fields() -> TestResult {
    ...
    // telemetry fields alongside, which the handler ignores.
    shims.write(
        "claude",
        &format!(
...
fn dev_output_matching_neither_shape_is_terminal_with_the_output_head() -> TestResult {
    let shims = Shims::new()?;
    shims.write("claude", "printf '%s' 'claude exploded mid-flight'")?;
    ...
    .err()
    .ok_or("unparseable claude output must fail the activity")?;
    assert_terminal(&failure, "claude dev produced unparseable output");
    assert_terminal(&failure, "claude exploded mid-flight");
    Ok(())
}
