//// The onatopp-dev child workflow: concurrent warm-build + dev start-up,
...
//// `stacked_dev` and is independently dispatchable as its own `[[workflow]]`
...
import stacked_dev/types.{
  ...
  type OnatoppResult, CheckFail, CheckPass, CheckResult, DevInput, Developed,
  OnatoppResult, OnatoppStageFailed, OnatoppStatus, ResumeInput, ScopedInput,
...
/// this module's name.
pub const workflow_type = "onatopp_dev"

/// Name of the live `{phase, round}` status query this workflow answers.
pub const status_query_name = "onatopp_dev_status"
...
/// loop.
pub fn execute(input: OnatoppInput) -> Result(OnatoppResult, OnatoppError) {
...
fn verify_loop(
  ...
  round: Int,
) -> Result(OnatoppResult, OnatoppError) {
...
/// `onatopp_dev_status` queries answer live state at every yield point
...
fn result_try(
  result: Result(value, OnatoppError),
  next: fn(value) -> Result(output, OnatoppError),
) -> Result(output, OnatoppError) {
  case result {
    Ok(value) -> next(value)
    Error(onatopp_error) -> Error(onatopp_error)
  }
}
