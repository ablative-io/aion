...
fn dev_parses_the_canned_dev_report() -> TestResult {
    ...
    assert!(
        log.contains("--print --fast --reasoning-effort high --session-id stacked-dev-brief-7")
    );
...
fn dev_resume_resumes_the_session_and_carries_the_feedback() -> TestResult {
    ...
    let log = shims.log("norn");
    assert!(log.contains("--print --fast --reasoning-effort high --resume stacked-dev-brief-7"));
    assert!(
...
fn request_review_sends_collective_dm_to_each_reviewer() -> TestResult {
    ...
    let acked = handlers::request_review(
        ...
        &ReviewRequest {
            ...
            dev_result: stacked_dev_worker_norn::types::DevResult {
                session_id: "stacked-dev-brief-7".to_owned(),
                files_touched: Vec::new(),
                summary: "implemented the brief".to_owned(),
            },
            gate_result: stacked_dev_worker_norn::types::GateResult {
...
fn assemble(
    design: &Design,
    wave: &[&str],
) -> Result<stacked_dev_worker_norn::types::AssembledWave, ActivityFailure> {
