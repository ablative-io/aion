...
//!
//! The defect: `provision_workspace` cloned into a volatile OS temp
//! directory (`mktemp -d`), the path was recorded in durable workflow
//! history, and a host reboot or temp-reaper deleted it — resume then
//! re-dispatched activities against the dead path and every unpushed
//! dev-round commit was lost (the branch existed on no machine).
//!
//! The loss-repro test drives the real handlers against a real `git` on
//! `PATH`: provision from a `file://`-style local clone URL, commit a
//! simulated dev round in the workspace, simulate the reboot purge (delete
//! the workspace IFF it lives under a volatile temp root), then run a later
//! activity step against the recorded `workspace.path` and require both the
//! step to succeed and the commit to be intact.
//!
...
//! re-provisions with the same id, run_id-less payloads get a unique
//! per-attempt directory and never collide with a salvaged one, path
//! traversal in the run key is refused, teardown refuses paths outside the
//! (canonicalized) root, and a genuinely missing workspace fails with the
//! explicit "workspace missing" diagnostic.
...
/// crate's own `target/`, which is NOT a volatile OS temp root, so the
/// simulated reboot purge must leave it alone.
#[allow(unsafe_code)]
...
/// crate shape lets the warm-build activity genuinely succeed post-purge.
...
/// purges: the OS temp dir, `/tmp`, or `/var/folders` (macOS per-user temp),
...
/// A remote-clone provision input against the fixture repository, keyed by
/// a stable per-run id like the workflow threads from `workflow.id()`.
...
/// with the SAME run id instead of wedging terminally.
#[test]
fn run_id_collision_renames_the_stale_attempt_aside_and_reprovisions() -> TestResult {
    ...
    let workspace =
        handlers::provision_workspace(&shell, clone_input(&fixture, "collision", "run-collision"))
            .map_err(|failure| {
                format!(
