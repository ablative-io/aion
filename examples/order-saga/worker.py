...
registers all order fulfillment saga activities and serves tasks until
interrupted. Set SIMULATE_SHIPPING_FAILURE=true to exercise compensation.
"""
...
class OrderSagaDispatcher:
    ...
    async def confirm_order(self, request: dict[str, object]) -> DispatchOutcome:
        order_id = required_string(request, "order_id")
        shipment_id = required_string(request, "shipment_id")
        confirmation_id = f"conf-{order_id}"
        logging.info(
            "Confirming order: order_id=%s shipment_id=%s confirmation_id=%s",
            order_id,
            shipment_id,
            confirmation_id,
        )
