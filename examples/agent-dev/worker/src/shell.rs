//! Typed shell-out boundary, reused from the stacked-dev-remote worker's
//! `shell.rs` (the #175-hardened seam).
//!
...
//! the caller interprets (gate diagnostics here). Output is the combined
//! stdout/stderr text (stdout first).
...
impl CliFailure {
    /// Render the failure as a single diagnostic line.
    #[must_use]
...
impl Shell {
    ...
    /// Resolve `executable` against the effective search path.
