//! Typed shell-out boundary, copied from stacked-dev's norn-worker
//! (`examples/stacked-dev/norn-worker/src/shell.rs`).
//!
...
//! the caller interprets. Output is the combined stdout/stderr text (stdout
//! first), matching the Erlang port runner's `stderr_to_stdout` merge
//! whenever one stream is silent.
