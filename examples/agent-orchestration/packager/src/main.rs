...
fn read_compiled_beams(workflow_root: &Path) -> Result<BeamSet> {
    let ebin = workflow_root.join("build/dev/erlang/aion_agent_orchestration/ebin");
    if !ebin.exists() {
        bail!(
            "compiled BEAM directory {} does not exist; run `gleam build` in examples/agent-orchestration first",
            ebin.display()
        );
    }
