...
pub struct ProvisionInput {
    /// Absolute path of the repository to provision from (local), or a
    /// working directory for the clone (remote).
    ...
    /// Git clone URL for remote provisioning. Required when
...
pub struct LandInput {
    ...
    pub dev_result: DevResult,
    /// Git remote URL for push+PR landing (remote placement). Absent for
    /// local runs that land via `yg branch merge`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub clone_url: Option<String>,
}
