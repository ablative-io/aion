...
- *(2026-07-24, Tom)* **Run #2 (`cb71b1a4`) cancelled; fixes-before-relaunch.** L01's developer turn died on a transient provider stream error — the harness classified it retryable, but no AWL `retry` policy was declared, so the activity was run-exactly-once and the failure would have killed the whole packet at the join. Four platform fixes landed before run #3: (1) per-token provider frames (`event/raw` `*.delta`) are now ephemeral, never persisted — the flood wrote 16 GB of store garbage in 80 minutes; (2) the observability tap drains through one ordered queue and fails cheap (16-retry budget, was 1024); (3) every remediation-packet activity now declares `retry` (agent turns `5 backoff 30s..10m` — retry resumes the norn session; shell `3 every 30s`); (4) workflow failure details now carry the underlying activity error instead of `"activity failed"`. Wave 1 lanes redispatch from scratch on relaunch (no branches were pushed).
