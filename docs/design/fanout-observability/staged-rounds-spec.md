...
11. `README.md`: composition diagram (model `examples/dev-brief/README.md:12-51`), the node table, deploy/run instructions — direct path only: `aion deploy examples/staged-rounds/awl/staged_rounds.awl`, `aion start staged_rounds --input … --task-queue staged_rounds` or one-motion `aion run … --input …`; worker launch line with `--profiles-dir ./profiles --max-parallel 4 --ready-file <path>`; **manual start, no launchd** (Tom's standing rule).
