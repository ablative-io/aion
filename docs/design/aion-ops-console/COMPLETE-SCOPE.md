...
- **G3 — Rendered 29-event + cluster + command + status-projection reference.** 184 ts-rs doc-comment lines sit unused. AC: auto-generated event reference (grouped, envelope + key fields) wired into the `export_ops_console_wire_types` discipline so it can't drift; cluster/command references flagged "typed but not emitted"/"handler unimplemented"; status-as-projection explained. Effort M. Severity P0 (events) / P1 (rest). Deps types.
