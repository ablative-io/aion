...
**NOI-6 landed** (`0ca827a7`, re-gated on a forced `--all-features` recompile: e2e + 6 server + 88
worker intervention tests green, both negative controls, `no-norn-in-platform` clean). **Next: NOI-7**
(ops-console `TranscriptPanel` + capability-gated `InterventionControls`) → NOI-8 (2nd adapter, SDK
generality proof). Two committed strategic design docs now sit beside this one:
`ZERO-CONFIG-CLUSTER-FORMATION.md` and `WORKER-DEPLOYMENT.md`, both gated on #146. Update after each merge.
