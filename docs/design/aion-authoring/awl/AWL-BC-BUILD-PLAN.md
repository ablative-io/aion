...
|-------|------|--------|---------------|
| **BC-0** | `aion_flow` hoist + descriptor-driven codec engine (D-BC2); re-point the Gleam emitter at the SDK helpers; re-baseline goldens; regenerate committed example modules | full aion-awl/aion-cli suites; compile proofs; awl-hello e2e trail unchanged; generated-module line count drops ≥40% | Opus implements, Fable panel + design of the descriptor value shape |
...
| **BC-2** | MIR + `lower` from rev-2 `CheckedDocument`; `AWL-BC-IR.md` (D-AOT2); `.gleam_types` emission from TypeEnv (D-AOT1) | MIR golden files per fixture; sidecar goldens; IR doc review | Fable designs the MIR (competing designs + judge panel), Opus implements lower against the ratified design |
...
| **BC-4** | Differential harness (net-new): every fixture both backends accept, through a real engine, identical durable event trails after run-id/`recorded_at` normalization; ABI contract tests (draft §4 table); adversarial corpus re-expressed in rev-2 (empty fork collection, runtime-sized fork, nested handlers, timeout-inside-retry, unicode, max-arity records, zero-step workflow, child spawn) | the normalized trails, byte-for-byte | Opus builds harness, Fable adjudicates any divergence |
