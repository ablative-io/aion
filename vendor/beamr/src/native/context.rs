...
pub enum NativeContinuation {
    ...
    /// Extension continuation for Aion's with_timeout operation trampoline.
    AionTimeout(AionTimeoutContinuation),
}

/// Host callback used by Aion's with_timeout continuation.
pub type AionTimeoutResumeFn =
    fn(AionTimeoutContinuation, Term, &mut ProcessContext) -> Result<ContinuationStep, Term>;

/// Continuation state for Aion's with_timeout operation trampoline.
#[derive(Clone, Debug)]
pub struct AionTimeoutContinuation {
    /// Opaque host state identifier resolved by the host callback.
    pub state_id: u64,
    /// Host resume callback.
    pub resume: AionTimeoutResumeFn,
}
