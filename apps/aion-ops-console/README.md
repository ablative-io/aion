...
The ops console is part of the product: `crates/aion-server/ops-console-embed/` is
a **committed** bundle and every build embeds it. You only run the pipeline to
**refresh** that committed bundle after changing the ops-console source:
...
./target/release/aion server --open   # serves the ops console at the HTTP port
```

**The bundle is committed.** `crates/aion-server/ops-console-embed/` holds the real
...
### CI guards (`.github/workflows/ops-console-embed.yml`)
...
2. **embed-freshness** — `cargo xtask verify-ops-console` rebuilds the ops console
   into a scratch dir and diffs it against the committed `ops-console-embed/`
...
The generation target for this app is `apps/aion-ops-console/src/types/generated/`. The public
