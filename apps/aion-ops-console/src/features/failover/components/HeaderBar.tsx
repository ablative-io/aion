...
export function HeaderBar({ status, namespace, activeNodeLabel, failedOver }: HeaderBarProps) {
  return (
    <header className="flex flex-col gap-4 border-border border-b pb-4 md:flex-row md:items-center md:justify-between">
      <div className="space-y-1">
        <p className="font-medium text-[0.7rem] text-muted-foreground uppercase tracking-[0.3em]">
          AION
        </p>
        <h1 className="font-semibold text-2xl text-foreground tracking-tight">FAILOVER</h1>
      </div>
      <div className="flex flex-wrap items-end gap-4">
        <div className="flex flex-col gap-1">
          <span className="font-medium text-muted-foreground text-xs">Read target</span>
          <span
            className="font-mono text-sm"
            data-failed-over={failedOver}
            style={{ color: failedOver ? 'var(--status-danger)' : 'var(--text-secondary)' }}
          >
        ...
        <div className="flex flex-col gap-1">
          <span className="font-medium text-muted-foreground text-xs">Namespace</span>
          <span className="font-mono text-secondary-foreground text-sm">ns: {namespace}</span>
        </div>
