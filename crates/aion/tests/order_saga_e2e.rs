...
//!
//! The archives are built from the committed example source on every run
...
/// Build both saga packages from the committed example source.
fn saga_packages() -> Result<(Package, Package), Box<dyn std::error::Error>> {
    let parent = example_build::built_package("examples/order-fulfillment", "order_fulfillment")?;
    let child = example_build::built_package("examples/order-fulfillment", "order_shipping")?;
    Ok((parent, child))
}
