...
/// One `pid` generation's permanent exit record and tracked observer.
pub(super) struct ProcessExitRecord {
    ...
    beamr_consumptions: AtomicUsize,
    #[cfg(test)]
...
impl ProcessExitRecord {
    fn new(
        ...
        Self {
            ...
            beamr_consumptions: AtomicUsize::new(0),
            #[cfg(test)]
    ...
    fn observe(self: &Arc<Self>, scheduler: &Scheduler) {
            ...
            && (forced_unavailable || scheduler.peek_exit_reason(self.pid).is_none())
        {
            ...
            self.beamr_consumptions.fetch_add(1, Ordering::AcqRel);
...
/// Runtime-lifetime index of owned exit records.
...
impl ProcessExitRegistry {
    ...
    #[cfg(test)]
    pub(super) fn beamr_consumptions(&self, pid: Pid) -> Result<usize, EngineError> {
        Ok(self.get(pid)?.beamr_consumptions.load(Ordering::Acquire))
    }
...
impl RuntimeHandle {
    ...
    pub(crate) fn exit_outcome_consumptions_for_test(
        &self,
        pid: Pid,
    ) -> Result<usize, EngineError> {
        self.process_exits.beamr_consumptions(pid)
    }
