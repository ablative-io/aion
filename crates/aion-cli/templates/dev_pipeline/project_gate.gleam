...
//// the dev child is the inner iteration aid; this gate is the trustworthy
...
/// this module's name.
pub const workflow_type = "{{name}}_gate"
...
pub fn run(raw_input: Dynamic) -> Result(String, String) {
  case decode.run(raw_input, decode.string) {
    Ok(raw_json) ->
      case codecs_workflows.gate_input_codec().decode(raw_json) {
        Ok(input) ->
          case execute(input) {
            Ok(output) -> Ok(codecs_workflows.gate_result_codec().encode(output))
            Error(gate_error) ->
              Error(codecs_workflows.gate_error_codec().encode(gate_error))
          }
        Error(codec.DecodeError(reason: reason, path: _)) ->
          Error(
            codecs_workflows.gate_error_codec().encode(GateStageFailed(
              stage: "decode_input",
